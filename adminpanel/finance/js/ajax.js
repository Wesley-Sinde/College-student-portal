// Admin Log in
$(document).on("submit", "#adminfinanceLoginFrm", function () {
  $.post(
    "query/loginExe.php",
    $(this).serialize(),
    function (data) {
  ...
  $.post(
    ...
    function (data) {
      if (data.res == "exist") {
        ...
        window.location.href = "home.php?page=AddCourseUnits";
        Swal.fire(
          "Success",
...
// Add Exam downloadable
$(document).on("submit", "#addExamdownloadable", function () {
  Swal.fire("No Course1", "Please select course", "error");
  $.post(
    "query/addExamDownloadableExe.php",
    $(this).serialize(),
    function (data) {
      if (data.res == "noCourse") {
        Swal.fire("No Course", "Please select course", "error");
      }
      if (data.res == "noUnit_Code") {
        Swal.fire("No Unit_Code Given", "Please input Unit code", "error");
      } else if (data.res == "noFile") {
        Swal.fire("No file chosen", "Choose a file before you submit", "error");
      } else if (data.res == "success") {
        Swal.fire(
          "Success",
          data.examTitle.toUpperCase() + "<br>Successfully Added",
